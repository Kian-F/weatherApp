...
class CityDetails extends Component {
  ...
  componentDidMount = () => {
    ...
    axios.get(weatherURL + this.props.match.params.id).then((result) => {
      console.log({ result });
      const fiveDays = result.data.sun_rise;
      console.log(fiveDays);

      this.setState({
        weatherDataDetails: result.data.consolidated_weather,
        sunRise: result.data,
      });
    });
  };
            ...
            <div key={index} className="col-auto">
              <div className="card bg-light">
                ...
                <p className="text-muted">
                  {moment(day.applicable_date).format("MMM Do YYYY")}
                </p>
                ...
                <div className="card-body">
                  <p className="card-text"> {day.weather_state_name}</p>
                  <p className="card-text">
                    {" "}
                    <span style={{ fontWeight: "bold", color: "#44aadd" }}>
                      {" "}
                      Max:
                    </span>{" "}
                    {day.max_temp.toFixed(0) + "°C"}
                  </p>
                  <p className="card-text">
                    {" "}
                    <span style={{ fontWeight: "bold", color: "#44aadd" }}>
